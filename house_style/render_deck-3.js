...
const textH = (text, w, size) => { const cpl = Math.max(8, Math.floor(w / ((size / 72) * 0.50)));
...
const stack = (needs, top, bottom, gap = 0.12, cap = CAPH) => {
...
const pack = (list, needOf, room, gap = 0.12, g = 1) => {
...
const packGrid = (list, cols, needOf, room, gap) => {
...
const needText = (t, w, size = MIN) => Math.max(ROWMIN, textH(t, w, size) + PAD);
const needShape = it => needText(`${it[0]}  (${it[1]})`, 8.3);
const needReveal = it => needText(it.q, 8.0) + textH(it.a, 8.0, ANS) + 0.14;
const R = {};
...
R.handout = (sp, next) => { const s = pres.addSlide(); frame(s, "Your worksheet", "Hand out the worksheet now", next);
  ...
  const parts = L.worksheet.parts, box = stack(parts.map(p => textH(`Part ${p.id}  ${p.title}`, 8.6, MIN) + 0.06), y0, BOT, 0.05);
...
R.vocab = (sp, next) => { const s = pres.addSlide(); frame(s, "Vocabulary" + (sp.cont ? " \u00B7 cont." : ""), sp.head || "Key vocabulary", next);
  ...
  const rowNeed = []; for (let r = 0; r < rows; r++) rowNeed.push(Math.max(...[0, 1].map(c => items[r * 2 + c] ? textH(items[r * 2 + c][1], cw - 0.5, MIN) + 0.74 : 0)));
...
function matchBody(s, items, bank, upto) {
  const bankTxt = bank.join("   \u00B7   "), bh = textH(bankTxt, 8.7, MIN) + 0.44;
  const box = stack(items.map(v => needText(v[1], 5.4)), TOP + bh + 0.12, BOT, 0.1);
  card(s, 0.4, TOP, 9.2, bh, TINT, TLINE);
  T(s, "WORD BANK", 0.64, TOP + 0.06, 2.2, 0.24, { ref: true, fontSize: 11, bold: true, color: ACC, charSpacing: 2 });
  T(s, bankTxt, 0.64, TOP + 0.3, 8.7, bh - 0.36, { fontSize: MIN, bold: true, color: NAVY });
  items.forEach((v, i) => { const { y, h } = box[i], done = i < upto;
    card(s, 0.4, y, 9.2, h, done ? TINT : PALE, done ? TLINE : LINE);
...
R.howto = (sp, next) => { const p = part(sp.part), s = pres.addSlide();
  frame(s, "Worksheet walkthrough", `Part ${p.id} \u2014 ${p.title}`, next);
...
R.walk = (sp, next) => { const p = part(sp.part), s = pres.addSlide();
  frame(s, "Worksheet walkthrough" + (sp.cont ? " \u00B7 cont." : ""), `Part ${p.id} \u2014 ${p.title}`, next);
  ...
  const tw = iw - 1.05;
  const box = stack(items.map(it => needText(it.q, tw) + 0.1), y, BOT, 0.1);
  items.forEach((it, i) => { const { y: yy, h } = box[i];
    card(s, 0.4, yy, iw, h); numDot(s, sp.from + i + 1, 0.6, yy + (h - 0.5) / 2, 0.5);
    T(s, it.q, 1.3, yy + 0.05, tw, h - 0.1, { fontSize: MIN, valign: "middle" }); }); };

R.reveal = (sp, next, i) => { const p = part(sp.part), items = sp.slice, s = pres.addSlide();
  frame(s, i < 0 ? "The questions" : `Answers \u00B7 ${i + 1} of ${items.length}`,
    (i < 0 ? `Part ${p.id} \u2014 ${p.title}` : `Part ${p.id} answers \u2014 ${p.title}`), next);
  const box = stack(items.map(it => needReveal(it)), TOP, BOT, 0.1);
  items.forEach((it, k) => { const { y, h } = box[k], done = k <= i, qh = needText(it.q, 8.0) - PAD;
    card(s, 0.4, y, 9.2, h, done ? TINT : PALE, done ? TLINE : LINE);
    numDot(s, sp.from + k + 1, 0.62, y + (h - 0.5) / 2, 0.5, done ? BLUE : "9DB3C7");
    T(s, it.q, 1.3, y + 0.1, 8.0, qh, { fontSize: MIN });
    if (done) T(s, it.a || "\u2014", 1.3, y + 0.14 + qh, 8.0, h - qh - 0.22, { fontSize: ANS, bold: true, color: BLUE }); }); };
...
R.rows = (sp, next) => { const s = pres.addSlide(); frame(s, sp.tag + (sp.cont ? " \u00B7 cont." : ""), sp.head, next);
  ...
  const lw = Math.min(Math.max(lwNeed, ic ? 1.5 : 2.0), w * 0.42), tw = w - lw - 0.56;
  const box = stack(items.map(r => needText(r[1], tw)), TOP, BOT);
...
R.speaking = (sp, next) => { const s = pres.addSlide(); frame(s, "Speaking", sp.head, next);
  ...
  if (eo && !sp.cont) englishOnly(s, 0.65, TOP + 1.9);
...
const NEEDOF = { goals: () => (g => needText(g, 7.9)), warmup: () => (q => textH(q, 7.9, MIN) + 0.3),
  rows: sp => (r => needText(r[1], (9.6 - (sp.icon ? 3.1 : 0.4)) - (sp.icon ? 1.5 : 2.4) - 0.56)),
  cards: sp => (c => needText(c.body, (9.6 - (sp.icon ? 2.9 : 0.4)) - 0.45) + 0.74),
  vocab: () => (v => needText(v[1], 4.1) + 0.72), wordlist: () => (w => needText(w[1], 6.25)), speaking: () => (f => textH(f, 6.22, MIN) + 0.14) };
const BAND = { warmup: sp => (BOT - TOP) - (sp.note ? textH(sp.note, 8.6, MIN) + 0.4 : 0), speaking: sp => BOT - TOP - (textH(sp.prompt, 6.22, MIN) + 0.41) - 0.58 };
...
function expand(sp) {
  ...
  if (sp.type === "walk") { const p = part(sp.part), its = wsItems(p);
    ...
    let at = 0;
    return pack(its, it => needText(it.q, 8.15) + 0.1, BOT - TOP, 0.1).forEach((sl, i, a) => {
      const from = at; at += sl.length;
      push(`Part ${p.id} \u2014 ${p.title}`, n => R.walk({ ...sp, slice: sl, from, cont: i > 0 }, n), i < a.length - 1 ? null : undefined); }); }
  if (sp.type === "reveals") { const p = part(sp.part), its = wsItems(p).filter(x => x.a !== "");
    ...
    let at = 0;
    return pack(its, needReveal, BOT - TOP, 0.1).forEach(sl => { const from = at; at += sl.length;
      push(`Part ${sp.part} answers`, n => R.reveal({ ...sp, slice: sl, from }, n, -1), null);
      sl.forEach((_, i) => push(`Part ${sp.part} answers`,
        n => R.reveal({ ...sp, slice: sl, from }, n, i), i < sl.length - 1 ? null : undefined)); }); }
  if (sp.type === "match") {
    const bankTxt = (sp.bank || sp.words.map(w => w[0])).join("   \u00B7   ");
    const bh = textH(bankTxt, 8.7, MIN) + 0.42;
    return pack(sp.words, v => needText(v[1], 5.4), BOT - TOP - bh - 0.12, 0.1).forEach(sl => {
      const bank = sp.bank || sp.words.map(w => w[0]);
  ...
  const groups = sp.type === "vocab" ? packGrid(list, 2, v => textH(v[1], 4.1, MIN) + 0.74, BOT - TOP, 0.12)
    : NEEDOF[sp.type] ? pack(list, NEEDOF[sp.type](sp), BAND[sp.type] ? BAND[sp.type](sp) : BOT - TOP, 0.12, sp.pair || 1)
    : balanced(list, PER[sp.type] || list.length);
  let at = 0;
  return groups.forEach((sl, i, a) => { const from = at; at += sl.length;
    push(lab, n => R[sp.type]({ ...sp, slice: sl, from, cont: i > 0 }, n), i < a.length - 1 ? null : undefined); });
}
